    ...
    public async execute(message: IMessage, args: string[]): Promise<any> {
        ...
        if (/^https?:\/\/((www|music)\.youtube\.com|youtube.com)\/playlist(.*)$/.exec(url)) {
            try {
                ...
                return message.channel.send(
                    createEmbed("info", `✅ **|** All videos in **[${playlist.title}](${playlist.url})** DAFTAR PUTAR TELAH DI TAMBAHKAN KE ANTRIAN`)
                        .setThumbnail(playlist.thumbnailURL)
                );
            } catch (e) {
    ...
    private async handleVideo(video: Video, message: IMessage, voiceChannel: VoiceChannel, playlist = false): Promise<any> {
        ...
        if (message.guild?.queue) {
            if (!this.client.config.allowDuplicate && message.guild.queue.songs.find(s => s.id === song.id)) {
                ...
                return message.channel.send(
                    createEmbed("warn", `🎶 **|** **[${song.title}](${song.url})** TELAH BERADA DI ANTRIAN, ` +
            ...
            if (!playlist) {
                message.channel.send(createEmbed("info", `✅ **|** **[${song.title}](${song.url})** TELAH DITAMBAHKAN KE ANTRIAN`).setThumbnail(song.thumbnail))
                    .catch(e => this.client.logger.error("PLAY_CMD_ERR:", e));
            }
        } else {
    ...
    private async play(guild: IGuild): Promise<any> {
        ...
        if (!song) {
            ...
            serverQueue.textChannel?.send(
                createEmbed("info", `⏹ **|** MUSIC YANG ANDA MINTA TELAH SELESAI, SILAHKAN GUNAKAN **\`${guild.client.config.prefix}play\`** UNTUK MEMAINKAN MUSIK`)
            ).catch(e => this.client.logger.error("PLAY_ERR:", e));
            disconnectTimer = setTimeout(() => {
                serverQueue.connection?.disconnect();
                serverQueue.textChannel?.send(
                    createEmbed("info", `👋 **|** SAYA KELUAR DARI VOICE CHANNEL KARENA SAYA SENDIRIAN :( `)
        ...
        serverQueue.connection?.play(songData, { type: songData.info.canSkipFFmpeg ? "webm/opus" : "unknown", bitrate: "auto", highWaterMark: 1 })
            .on("start", () => {
                ...
                if (serverQueue.lastMusicMessageID !== null) serverQueue.textChannel?.messages.fetch(serverQueue.lastMusicMessageID, false).then(m => m.delete()).catch(e => this.client.logger.error("PLAY_ERR:", e));
                serverQueue.textChannel?.send(createEmbed("info", `▶ **|** SEDANG MEMUTAR: **[${song.title}](${song.url})**`).setThumbnail(song.thumbnail))
            ...
            .on("finish", () => {
                ...
                if (serverQueue.lastMusicMessageID !== null) serverQueue.textChannel?.messages.fetch(serverQueue.lastMusicMessageID, false).then(m => m.delete()).catch(e => this.client.logger.error("PLAY_ERR:", e));
                serverQueue.textChannel?.send(createEmbed("info", `⏹ **|** BERHENTI MEMUTAR: **[${song.title}](${song.url})**`).setThumbnail(song.thumbnail))
                    .then(m => serverQueue.lastMusicMessageID = m.id)
                    .catch(e => this.client.logger.error("PLAY_ERR:", e))
                    .finally(() => {
